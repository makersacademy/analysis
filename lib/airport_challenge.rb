class AirportChallenge
  ...
  def rspec_tests
    @rspec_tests.nil? ? 0 : @rspec_tests.to_f 
  end
  def rspec_failures
    @rspec_failures.nil? ? 0 : @rspec_failures.to_f
  end
  def rubocop_offenses_detected
    @rubocop_offenses_detected.nil? ? 0 : @rubocop_offenses_detected.to_f 
  end
  def coverall_coverage
    @coverall_coverage.nil? ? 0 : @coverall_coverage.to_f
  end
  def reek_warnings
    @reek_warnings.nil? ? 0 : @reek_warnings.to_f
  end
end
